...
char *test_commands[] = {"ls", "cat", "/bin/sh", "./non_esisto", "comando_finto", NULL};

    for (int i = 0; test_commands[i] != NULL; i++) {
        printf("Testando: %s\n", test_commands[i]);
        
        char *res = which(test_commands[i]);
        
        if (res) {
            printf("  SUCCESS: Trovato in -> %s\n", res);
            free(res); 
        } else {
            printf("  FAILURE: Comando non trovato.\n");
        }
        printf("---------------------------\n");
    }
